...
function initSky(id,img,array){
    ...
    // console.log(c.style.width.replace('px',''))
...
// 摩天旋转到一定角度（快到慢）
function startRotate(angel,baseStep,baseSpeed,array,img,callback){
...
function skyWheel(params){
    ...
    for(var i = 0 ; i < options.num;i++){
        ...
        // console.log(options)
        try {
            ctx.drawImage(options.img,xr-options.award_width/2,yr,options.award_width,options.award_height);       
        ...
        if(options.award[i]&&options.award[i].alt){
            //没有图片时
        ...
        // //上条线
        // ctx.moveTo(xr-options.award_width/2-4,yr+options.award_height+4)
        // ctx.lineTo(xr+options.award_width/2+4,yr+options.award_height+4)
        // // 左边圆
        // ctx.arc(xr+options.award_width/2+10,yr+options.award_height+15,11,-Math.PI/2,Math.PI/2);
        // // 下条线
