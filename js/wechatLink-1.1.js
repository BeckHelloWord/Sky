...
WeChatShare.appShare=function(){
    var shareStr="title="+WeChatShare.shareData.title+"&desc="+WeChatShare.shareData.desc+"&link="+WeChatShare.shareData.link+"&imgUrl="+WeChatShare.shareData.imgUrl
    var ua = navigator.userAgent.toLowerCase();
    if(ua.indexOf('android') != -1){
        shareStr="title="+encodeURIComponent(WeChatShare.shareData.title)+"&desc="+encodeURIComponent(WeChatShare.shareData.desc)+"&link="+WeChatShare.shareData.link+"&imgUrl="+WeChatShare.shareData.imgUrl
    }
    try{
        var appFunc = JSON.parse(localStorage['appFunc']);
        if(appFunc&&appFunc.indexOf('share') != -1){
            window.location="baoxiang://share?"+shareStr
        }else{
